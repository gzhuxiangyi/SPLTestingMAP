...
public class DistancesUtil {
  ...
  //Used in sampling products
    public static double getWeightedDistance (Product p1, Product p2) {
        ...
        if (p1.getSelectedNumber() +  p2.getSelectedNumber() > MAP_test.getInstance().numFeatures) {
        	delta = 1.0 / ( MAP_test.getInstance().numFeatures - Math.min(p1.getSelectedNumber(), p2.getSelectedNumber()));
        }
        		
        dist = 0.5 *  (1.0 - intersectionSize / p1.size() ) * delta // getHammingDistance
        	 + 0.5 * ((double)(Math.abs(p1.getSelectedNumber() - p2.getSelectedNumber())) /(MAP_test.getInstance().numFeatures)) ;
        return dist;

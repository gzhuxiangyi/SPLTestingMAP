...
public class SPEA2Adaption {
    ...
    public SPEA2Adaption(int cellsInEachDim,long timeAllowedMS) {
    	this.cellsInEachDim = cellsInEachDim;
    	this.timeAllowedMS = timeAllowedMS;
    }
    
    public SPEA2Adaption(int lowerBound,int upperBound,long timeAllowedMS) {
...
//         while (Individual.counter < 6000) {       
           while (evaluations < 12000) {     
        	  ...
        	  SolutionSet union = population.union(archive);
        	  
        	  Spea2Fitness spea = new Spea2Fitness(union);
              spea.fitnessAssign();
              archive =  spea.environmentalSelection(populationSize);                     
...
//       //************The following code removes same solutions************
	       SolutionSet unique = new SolutionSet(firstFront.size());
	       unique.add(firstFront.get(0));
	       
	       for (int i = 1;i < firstFront.size();i++) {
	      	 IndividualMultiObj soli = firstFront.get(i);
       ...
       //************The above code removes same solutions************
         
	     unique.sort(new ObjectiveComparator(0,true));
         
         for (int i=0; i < unique.size();i++) {
        	 System.out.println("the " + i + "th test suite with (size,converage):(" 
        			 + unique.get(i).getSize() + "," + ( - unique.get(i).getObjective(0))+")");
         }
         
         return unique.getSolutionsList();
    }
